  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Slide image — full size, phone aspect ratio */}
      ...
      {/* Caption preview below thumbnails */}
      <div
...
export default function ContentOptions() {
  ...
  const handleApprovePublish = async (scriptId: number) => {
    setPublishing(scriptId);
    try {
      await fetch(`/api/queue/${scriptId}/approve`, { method: 'POST' });
      await fetch(`/api/queue/${scriptId}/publish`, {
        method: 'POST',
  ...
  const toUrl = (p: string): string => {
    ...
    if (assetsIdx !== -1) {
      return '/' + p.slice(assetsIdx).replace('data/', '');
    }
    return p;
  };

  const getAllSlides = (opt: OptionData): string[] => {
  ...
  const getThumbnail = (opt: OptionData): string | null => {
              ...
              return (
                <div
                  ...
                  className={`bg-gray-900 border rounded-xl overflow-hidden transition-all ${
                    batch.selectedIdeaId === opt.ideaId
                ...
                >
                  ...
                  <div className="p-4 space-y-2">
                    ...
                    <button
                      onClick={() => handleApprovePublish(opt.scriptId)}
                      disabled={publishing === opt.scriptId}
                      className={`w-full px-3 py-2 text-xs rounded-lg transition-colors font-medium ${
                        publishing === opt.scriptId
                          ? 'bg-gray-700 text-gray-400 cursor-not-allowed'
                          : 'bg-green-700 hover:bg-green-600 text-white'
                      }`}
                    >
                      {publishing === opt.scriptId ? 'Publishing...' : 'Approve & Publish'}
                    </button>
                  </div>
